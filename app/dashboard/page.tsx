...
export default function DashboardPage() {
  ...
  const getRiskColor = (score: number) => {
    if (score >= 8) return "text-red-600 bg-gradient-to-r from-red-50 to-red-100 border-red-200"
    if (score >= 6) return "text-yellow-600 bg-gradient-to-r from-yellow-50 to-yellow-100 border-yellow-200"
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <p className="text-lg text-slate-600">Analyze legal documents and understand what you're signing</p>
        </div>

        {/* Stats Cards */}
        <div
          className={`grid grid-cols-1 md:grid-cols-4 gap-6 mb-8 transition-all duration-1000 delay-200 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}
        >
          {[
            { title: "Total Analyses", value: stats.totalAnalyses, icon: FileText, color: "from-blue-500 to-cyan-500" },
            {
            ...
            { title: "Time Saved", value: stats.timesSaved, icon: AlertTriangle, color: "from-orange-500 to-red-500" },
          ].map((stat, index) => (
            <Card
              key={index}
        ...
        {/* Quick Actions */}
        <div
          className={`grid md:grid-cols-3 gap-6 mb-8 transition-all duration-1000 delay-400 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}
        >
        ...
        {/* Recent Analyses */}
        <Card
          className={`shadow-xl bg-white/80 backdrop-blur-sm border-0 transition-all duration-1000 delay-600 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}
        >
          <CardHeader>
            ...
            <CardDescription className="text-base">Your latest document analysis results</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {recentAnalyses.map((analysis, index) => (
                <div
                  key={analysis.id}
                  className={`group flex items-center justify-between p-6 border-2 rounded-xl hover:bg-slate-50 transition-all duration-300 hover:scale-102 hover:shadow-lg animate-fade-in-up delay-${index * 100}`}
                >
                  <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-center space-x-4">
                    <div
                      className={`px-4 py-2 rounded-full text-sm font-semibold border-2 transition-all duration-300 ${getRiskColor(analysis.riskScore)}`}
                    >
